...
var remainingGames = {
  '1_2': ['1_4', '1_5',  4], // team 1, team 2, point value
  '1_3': ['1_6', '1_7',  4],
  '2_2': ['2_4', '2_5',  4],
  '2_3': ['2_6', '2_7',  4],
  '3_2': ['3_4', '3_5',  4],
...
var remGames = ['1_2', '1_3', '2_2', '2_3', '3_2', '3_3', '4_2', '4_3', '1_1', '2_1', '3_1', '4_1', '0_2', '0_3', '0_1'];
var latestGames = {
  '1_4': 'Kansas',
  '1_5': 'Maryland',
  '1_6': 'Miami (FL)',
...
function simulateRest() {
  ...
  for (var i = 0; i < 65536; i++) {
    ...
    for (var j = 0; j < 15; j++) {
      var mask = 1 << j;
      var winner = (i & mask) === 0 ? 0 : 1;
      var id = remGames[j];
      var teams = remainingGames[id];
      var winningTeam = latestGames[teams[winner]];
      var losingTeam  = latestGames[teams[1-winner]];
      var points = teams[2];
      var winningSeed = remainingTeamSeeds[winningTeam];
      var losingSeed  = remainingTeamSeeds[losingTeam];
      if (winningSeed > losingSeed) {
        points += (winningSeed - losingSeed);
      }
      latestGames[id] = winningTeam;
      x.push(points + ' pts -- [' + winningSeed + '] ' + winningTeam + ' over [' + losingSeed + '] ' + losingTeam);
      res[id] = {team: winningTeam, points: points};
    }

    var win = { points: 0 };
    for (var r in picks) {
      // bracket
      //console.log(picks[r].name, picks[r].pot);
      var brack = picks[r].picks;
      var pts = picks[r].pot;
      for (var p in brack) {
        var pick = brack[p];
        if (pick.pick == latestGames[pick.id]) {
          pts += res[pick.id].points;
        }
        //console.log(pts, pick.pick, latestGames[pick.id]);
      }
      //console.log(picks[r].name, pts);
      if (pts > win.points) {
        win.points = pts;
        win.name = picks[r].name;
      }
    }

    numberOfWins[win.name] = numberOfWins[win.name] || 0;
    numberOfWins[win.name]++;

    //console.log(win.name, win.points);
  }
...
function get(id){
  ...
  req = req.then(function (htmlString) {
    var handler = new htmlparser.DefaultHandler(function (error, dom) {
      if (error) {
        ...
        console.log(util.inspect(picks[id], false, null));
      }
    });
  ...
  if (id === brackets.length - 1) {
    req.then(simulateRest);
  }
}
